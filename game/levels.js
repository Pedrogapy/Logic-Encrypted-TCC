...
function drawWalls(ctx,walls){
  for(const r of walls){
    ctx.fillStyle="#22283f"; ctx.fillRect(r.x,r.y,r.w,r.h);
    ctx.strokeStyle="#0e1324"; ctx.lineWidth=2; ctx.strokeRect(r.x+0.5,r.y+0.5,r.w-1,r.h-1);
  }
}
function drawTorches(ctx,torches){
  for(const t of (torches||[])){
    ...
    g.addColorStop(0,`rgba(255,180,80,${0.4*f})`); g.addColorStop(1,"rgba(0,0,0,0)");
    ctx.fillStyle=g; ctx.fillRect(t.x-110,t.y-110,220,220);
    ctx.fillStyle="#ffb454"; ctx.fillRect(t.x-3,t.y-3,6,6);
  }
}
...
export const LEVELS = [
  {
    ...
    map:{ w:1024,h:560,
      ...
      torches:[{x:120,y:116},{x:900,y:116}]
    },
    player:{x:80,y:400},
    door:{x:930,y:400,w:24,h:64},
    terminal:{x:520,y:380},
    pages:[{x:190,y:400,key:"variaveis"}] // 1 página
  },
  {
    ...
    map:{ w:1024,h:560,
      ...
      torches:[{x:160,y:116},{x:520,y:116},{x:880,y:116}]
    },
    player:{x:80,y:400},
    door:{x:930,y:120,w:24,h:64},
    terminal:{x:760,y:360},
    pages:[{x:400,y:420,key:"modulo"}] // **somente** 1 página
  }
];
...
export function buildLevel(def){
  const map = {
    ...
    draw(ctx){ drawGrid(ctx,this.w,this.h); drawWalls(ctx,this.walls); drawTorches(ctx,this.torches); }
  };
  const entities=[];
  for(const r of def.map.walls){
    entities.push({ x:r.x,y:r.y,w:r.w,h:r.h, solid:true,
      get bbox(){return {x:this.x,y:this.y,w:this.w,h:this.h};}, draw(){} });
  }
  ...
  let door=null; if(def.door){ door=new Door(def.door.x,def.door.y,def.door.w,def.door.h); entities.push(door); }
